...
public class TabasTrocCalActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ProgressBar carregandoProgressBar = findViewById(R.id.tabCircleProgressBar);

        //Roda isso no futuro, pra não ficar travando!
        ...
        Runnable carregoso = new Runnable() {
            ...
            public void run() {
                if (interrupted()) {
                    ...
                    SectionsPagerAdapter sectionsPagerAdapter = new SectionsPagerAdapter(getApplicationContext(), getSupportFragmentManager(), TabNames, listaFragmentosInflados);
                    viewPager.setAdapter(sectionsPagerAdapter);
                    TabLayout tabs = findViewById(R.id.tabs);
                    tabs.setupWithViewPager(viewPager);
                    carregandoProgressBar.setVisibility(View.INVISIBLE);
                }
            }
        };
        //Thread thread = new Thread(carregoso);
        temporal.postDelayed(carregoso, 1000);



        //Colocando como a appbar:
        ...
        if (intent != null) {
            if (intent.getIntExtra("tipoDeProjeto", 0) != 0) {
                ...
                mToolbar.setTitle(idNovoTitulo);
                getSupportActionBar().setTitle(idNovoTitulo);
                //try{tabTitle.setText(idNovoTitulo);} catch(Exception e){}
            }
        }
    ...
    public LinearLayout criarTodosOsErros(List<ErroPadrao> listaDeErros){
        ...
        //Primeiro coloca a imagem do gato quebrado
        LinearLayout imagemGato = (LinearLayout) getLayoutInflater().inflate(R.layout.erro_layout_gato_1, null);
        //Depois a lista de erros
        linLay.addView(imagemGato);
