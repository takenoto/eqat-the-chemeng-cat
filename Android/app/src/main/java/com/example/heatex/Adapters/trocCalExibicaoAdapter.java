...
public class trocCalExibicaoAdapter extends RecyclerView.Adapter<trocCalExibicaoAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        if(holder.idTipoTrocador == TrocadorBitubularHeatex.BITUBULAR){
            ...
            holder.tipoProjeto.setText(R.string.trocador_bitubular);
        }
        //Uma vez isso feito, arrumar os outros:
        else if(holder.idTipoTrocador == TrocadorBitubularHeatex.MULTITUBULAR){
            var = troquinho.getLDiDsDp(0);
            valores = troquinho.getValoresLDiDsDp();
            unidades = troquinho.getLDiDsDp(1);
            holder.tipoProjeto.setText(R.string.trocador_de_calor_multitubular_hairpin);

        }else if(holder.idTipoTrocador == TrocadorBitubularHeatex.CASCOETUBOS){
            var = troquinho.getLDiDsDp(0);
            valores = troquinho.getValoresLDiDsDp();
            unidades = troquinho.getLDiDsDp(1);
            holder.tipoProjeto.setText(R.string.trocador_casco_e_tubos);
        }
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        int idTipoTrocador =TrocadorBitubularHeatex.BITUBULAR;
        ...
        public MyViewHolder(View itemView) {
            ...
            if(idTipoTrocador == TrocadorBitubularHeatex.BITUBULAR){
                valores = listaTrocs.get(0).getValoresLDiDsDp();
            }else if(idTipoTrocador == TrocadorBitubularHeatex.MULTITUBULAR) {
                valores = listaTrocs.get(0).getValoresLDiDsDp();
            }
            else if(idTipoTrocador ==TrocadorBitubularHeatex.CASCOETUBOS){
                valores = listaTrocs.get(0).getValoresLDiDsDp();
            }
