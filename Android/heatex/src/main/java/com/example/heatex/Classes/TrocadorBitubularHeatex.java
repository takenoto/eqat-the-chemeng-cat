...
public class TrocadorBitubularHeatex{
    ...
    public int idQueDefineTipoTrocador = R.string.trocador_bitubular; // mudar depois!
    ...
    public static final String[] correlacoesDisponiveis = {"TODAS","UWT", "UHT", "Dittus-Boelter", "Sieder-Tate(Lam)", "Sieder-Tate(Turb)", "PetukhovPopov", "Hausen(transition)", "NotterCleicher", "Seban & Shimazaki", "Gnielisnk"};
    ...
    public void projetar(){
        ...
        ΔTlm = Math.abs(   (ΔT1 - ΔT2)/(Math.log(ΔT1/ΔT2))   );
        ...
        for(int i=0; i<=10000; i++){
            ...
            }else{L_hairpin =  (int)(1 + 10*Latual/(2*n_hairpin))/10.0;} //arredonda 1 casa decimal

            //Determina o comprimento total atual
            this.L_total = n_hairpin*L_hairpin; //salva como comprimento máximo
            ...
            if(!hiEhConstante){
                ...
                switch (correlacaoTubo){
                    case UWT:
                        Nu_i = getNuUWT(tipoFluido1, Pe, Re_i, Pr_i);
                        break;
            ...
            if(!hoEhConstante){
                ...
                switch (correlacaoAnulo){
                    case UWT:
                        Nu_o = getNuUWT(tipoFluido2, Peo, Re_o, Pr_o);
                        break;
            ...
            //->Tubo
            double ΔPi_dist = 4*L_hairpin*n_hairpin*n_tubos*ff_i*(G_i*G_i)/(Di*ρ_i);
    ...
    double getALinear(double Do){return Math.PI*Do;}
